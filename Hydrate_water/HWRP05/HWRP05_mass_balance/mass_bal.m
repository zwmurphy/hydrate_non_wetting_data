...
V_dead=295; %volume of line btw pump and core (ml)
...
deltaNm(ii)=((((Vpump(1)+V_dead)*10^-6*P(1)*6894.75)/((T(1)+273.15)*R))-(((Vpump(ii)+V_dead)*10^-6*P(ii)*6894.75)/((T(2)+273.15)*R)))*Mm;  % mass in gas (kg)
end


%% Experimental initial conditions (MUST KNOW THESE)!!!!!!!!!!
Sli=0.75; %inital brine saturation (know from mass balance/initial saturation procedure)
Sgi=0.25; %initial gas saturation (know from mass balance/initial saturation procedure)
Shi=1-Sli-Sgi; %initial hydrate saturation (Should be 0)
Xsli=0.035; Xwli=1-Xsli; % initial salinity of brine
...
% 
% yyaxis left
% plot(t,Sg2,'-r','Linewidth',1.25)
% plot(t,Sl2,'-b','Linewidth',1.25)
% plot(t,Sh2,'-g','Linewidth',1.25)
...
%% Plot pressure, temp, vol
figure

subplot(3,1,1)
plot(t,Vpump,'Linewidth',1.25)
xlabel('time (hrs)'); ylabel('Volume CH_4 in pump (ml)')
set(gca, ...
  'FontName','Arial',...
  'FontSize',18,...
  'Box'         , 'off'     , ...
...
yyaxis left
hold on
plot(t,P_up,'k','Linewidth',1.25)
plot(t,P_down,'r','Linewidth',1.25)
hold off
xlabel('time (hrs)'); ylabel('Pressure (PSI)')
set(gca, ...
  'FontName','Arial',...
  'FontSize',18,...
  'Box'         , 'off'     , ...
...
yyaxis right
plot(t,deltaP,'g','Linewidth',1.25)
ylabel('\delta P (psi)')
ylim([-300,300]);
legend('P upstream','P downstream','\deltaP','Location','best')
hold off
...
subplot(3,1,3)
plot(t,T,'Linewidth',1.25)
xlabel('time (hrs)'); ylabel('Temp (C)')
